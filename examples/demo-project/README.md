...
- `.harness/mcp.toml` — two MCP servers (a stdio one and an HTTP one) → transpiled to every tool's native shape
...
```bash
node dist/cli.js -C examples/demo-project apply      # generate native config for all 6 tools
